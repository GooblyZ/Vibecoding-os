...
import { useReveal, usePremiumButton } from "../lib/hooks";
...
const PRIMARY_HREF = "#contact";

// Inner light + press + ripple only (no magnetic — text link in card layout)
function StartBuildLink({ href, label }: { href: string; label: string }) {
  const linkRef = useRef<HTMLAnchorElement>(null);
  usePremiumButton(linkRef, { maxMagnet: 0 });
  return (
    <a
      ref={linkRef}
      href={href}
      aria-label={label}
      className="btn-arrow btn-premium font-mono text-[9px] tracking-widest uppercase text-[#5a6070] hover:text-[#00d4ff]/70 transition-colors duration-200 flex items-center gap-1.5 mt-auto pt-1 focus-visible:outline-none focus-visible:underline"
    >
      Start this build
      <span aria-hidden="true" className="arrow text-[8px]">→</span>
    </a>
  );
}
...
export default function FinalCTA() {
  ...
  usePremiumButton(startProjectRef);
  usePremiumButton(viewProjectsRef);
      ...
      <div className="relative z-10 max-w-7xl mx-auto" ref={sectionRef}>
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="reveal flex flex-col sm:flex-row gap-3">
              ...
              <a
                ref={startProjectRef}
                href={PRIMARY_HREF}
                className="btn-arrow btn-premium flex-1 inline-flex items-center justify-center gap-2 px-7 py-4 bg-[#00d4ff] text-[#080a0f] font-bold text-sm tracking-wide uppercase hover:bg-[#00d4ff]/90 transition-colors duration-200 rounded-sm glow-blue focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#00d4ff]/60"
              >
                Start a Project
              ...
              <a
                ref={viewProjectsRef}
                href="#projects"
                className="btn-premium flex-1 inline-flex items-center justify-center px-7 py-4 border border-white/12 text-[#8892a4] text-sm tracking-wide uppercase hover:border-white/25 hover:text-[#f0f0f0] transition-colors duration-200 rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/20"
              >
                View Projects
              </a>
            </div>
        ...
        <div className="grid sm:grid-cols-3 gap-4 mb-16 reveal">
          {PROJECT_TYPES.map((pt) => (
            <div
              key={pt.id}
              className="panel panel-hover rounded-sm p-6 flex flex-col gap-4 group relative overflow-hidden"
              onMouseMove={(e) => {
            ...
            >
              ...
              <StartBuildLink
                href={PRIMARY_HREF}
                label={`Start a ${pt.title} project`}
              />
            </div>
          ))}
        </div>
